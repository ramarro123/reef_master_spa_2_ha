...
// Setup LED FLash if LED pin is defined in camera_pins.h
  ...
  // RAMA EXTRA CONFIG
  Serial.println("Apply rama preference");
  s->set_quality(s, 10);
  ...
  s->set_denoise(s, 8);
  s->set_ae_level(s, 5);
  s->set_gainceiling(s, (gainceiling_t)114);
  s->set_special_effect(s, 1);

  s->set_hmirror(s, 1);
  s->set_vflip(s, 0);
  s->set_framesize(s, FRAMESIZE_SXGA);
  s->set_xclk(s, LEDC_TIMER_0, 13);


}

unsigned long get_fw_status_next = 0;
unsigned long wifimulti_next = 0;

void loop()
{
  unsigned long _mil = millis();

  if ((unsigned long)(_mil - get_fw_status_next) > 2000)
  {
    get_fw_status_next = _mil;
  ...
  uint8_t wmr = wifiMulti.run();
  if ((unsigned long)(_mil - wifimulti_next) > 5000)
  {
    wifimulti_next = _mil;
    if (wifiMulti.run() == WL_CONNECTED)
    {
